...
namespace Game
{
	public class SubsystemAA12Behavior : SubsystemBlockBehavior
	{
		public override int[] HandledBlocks
		{
			get
			{
				return new int[]
				{
					BlocksManager.GetBlockIndex(typeof(AA12Block), true, false)
				};
			}
		}
		...
		public override bool OnAim(Ray3 aim, ComponentMiner componentMiner, AimState state)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					...
					int num2 = slotValue;
					bool flag3 = num == BlocksManager.GetBlockIndex(typeof(AA12Block), true, false) && slotCount > 0;
					if (flag3)
					{
						double gameTime;
							...
							case AimState.InProgress:
								{
									...
									int bulletNum = AA12Block.GetBulletNum(Terrain.ExtractData(slotValue));
									bool flag9 = bulletNum > 0;
									// AA12 tiene cadencia de 300 RPM (0.2s por disparo)
									bool flag10 = this.m_subsystemTime.PeriodicGameTimeEvent(0.2, 0.0) && flag9;
									if (flag10)
									{
										...
										int num5 = 8; // 8 perdigones por disparo
										Vector3 vector5 = new Vector3(0.03f, 0.03f, 0.06f); // Mayor dispersión
										for (int i = 0; i < num5; i++)
										{
											int value = Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(NuevaBala6), true, false), 0, 2);
											...
											this.m_subsystemProjectiles.FireProjectile(value, vector2, 350f * (vector3 + v3), Vector3.Zero, componentMiner.ComponentCreature);
										}
										this.m_subsystemAudio.PlaySound("Audio/Armas/AA12 fuego", 1.5f, this.m_random.Float(-0.05f, 0.05f), componentMiner.ComponentCreature.ComponentCreatureModel.EyePosition, 15f, true);
										...
										this.m_subsystemNoise.MakeNoise(vector2, 1.2f, 45f);
										int bulletNum2 = bulletNum - 1;
										num2 = Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(AA12Block), true, false), 0, AA12Block.SetBulletNum(Terrain.ExtractData(slotValue), bulletNum2));
									...
									else
									{
										...
										if (flag13)
										{
											...
											{
												string bulletName = LanguageControl.Get("Blocks", "AA12BulletBlock:0", "DisplayName");
		...
		public override int GetProcessInventoryItemCapacity(IInventory inventory, int slotIndex, int value)
		{
			...
			int bulletNum = AA12Block.GetBulletNum(slotData);

			if (value == BlocksManager.GetBlockIndex(typeof(AA12BulletBlock), true, false) && bulletNum < 20)
			{
				return 1;
			}
		...
		public override void ProcessInventoryItem(IInventory inventory, int slotIndex, int value, int count, int processCount, out int processedValue, out int processedCount)
		{
			...
			if (bulletNum < 20)
			{
				processedValue = 0;
				processedCount = 0;
				int newData = AA12Block.SetBulletNum(slotData, 20);
				inventory.RemoveSlotItems(slotIndex, 1);
				inventory.AddSlotItems(slotIndex,
					Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(AA12Block), true, false), 0, newData), 1);
