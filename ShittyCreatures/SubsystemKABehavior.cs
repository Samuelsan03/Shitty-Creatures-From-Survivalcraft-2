...
namespace Game
{
	public class SubsystemKABehavior : SubsystemBlockBehavior
	{
		public override int[] HandledBlocks
		{
			get
			{
				return new int[]
				{
					BlocksManager.GetBlockIndex(typeof(KABlock), true, false)
				};
			}
		}
		...
		public override bool OnAim(Ray3 aim, ComponentMiner componentMiner, AimState state)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					...
					int num2 = slotValue;
					bool flag3 = num == BlocksManager.GetBlockIndex(typeof(KABlock), true, false) && slotCount > 0;
					if (flag3)
					{
						double gameTime;
							...
							case AimState.InProgress:
								{
									...
									int bulletNum = KABlock.GetBulletNum(Terrain.ExtractData(slotValue));
									bool flag9 = bulletNum > 0;
									bool flag10 = this.m_subsystemTime.PeriodicGameTimeEvent(0.1, 0.0) && flag9; // Ráfaga más rápida (0.1 vs 0.12)
									if (flag10)
									{
										...
										int num5 = 3; // Más balas por ráfaga
										Vector3 vector5 = new Vector3(0.007f, 0.007f, 0.03f); // Menor dispersión
										for (int i = 0; i < num5; i++)
										{
											int value = Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(NuevaBala5), true, false), 0, 0);
											...
											this.m_subsystemProjectiles.FireProjectile(value, vector2, 320f * (vector3 + v3), Vector3.Zero, componentMiner.ComponentCreature); // Mayor velocidad
										}
										this.m_subsystemAudio.PlaySound("Audio/Armas/KA fuego", 1f, this.m_random.Float(-0.1f, 0.1f), componentMiner.ComponentCreature.ComponentCreatureModel.EyePosition, 10f, true);
										...
										this.m_subsystemNoise.MakeNoise(vector2, 1f, 35f); // Menor ruido
										int bulletNum2 = bulletNum - 1;
										num2 = Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(KABlock), true, false), 0, KABlock.SetBulletNum(bulletNum2));
									...
									else
									{
										...
										if (flag13)
										{
											...
											{
												string bulletName = LanguageControl.Get("Blocks", "KABulletBlock:0", "DisplayName");
		...
		{
			int kaBlockIndex = BlocksManager.GetBlockIndex(typeof(KABlock), true, false);
			int kaBulletBlockIndex = BlocksManager.GetBlockIndex(typeof(KABulletBlock), true, false);

			if (value == kaBulletBlockIndex)
			{
				int slotValue = inventory.GetSlotValue(slotIndex);
				int slotContents = Terrain.ExtractContents(slotValue);

				if (slotContents == kaBlockIndex)
				{
					int data = Terrain.ExtractData(slotValue);
					int bulletNum = KABlock.GetBulletNum(data);
					return (bulletNum < 40) ? 1 : 0; // Capacidad de 40 balas
				}
			}
			return 0;
		}
			...
			if (value == kaBulletBlockIndex)
			{
				...
				if (slotContents == kaBlockIndex)
				{
					...
					if (bulletNum < 40)
					{
						...
						int newData = KABlock.SetBulletNum(40); // Recarga completa a 40
						int newValue = Terrain.MakeBlockValue(kaBlockIndex, 0, newData);
