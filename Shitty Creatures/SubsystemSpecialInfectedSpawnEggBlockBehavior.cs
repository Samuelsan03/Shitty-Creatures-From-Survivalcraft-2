...
namespace Game
{
	public class SubsystemSpecialInfectedSpawnEggBlockBehavior : SubsystemBlockBehavior
	{
		// SOLO especiales (tanques, jefes, etc.) - EXCLUYE voladores
		private static readonly string[] InfectedTemplates = new string[]
		{
			...
			"FrozenTank",
			"FrozenTankGhost"
            // NOTA: FlyingInfectedBoss NO está aquí (es volador)
        };
		...
		{
			get { return new int[] { SpecialInfectedSpawnEggBlock.Index }; }
		}
		...
		public override bool OnHitAsProjectile(CellFace? cellFace, ComponentBody componentBody, WorldItem worldItem)
		{
			...
			{
				Log.Error($"Error spawning special infected: {ex}");
				if (worldItem is Projectile projectile && projectile.Owner != null)
				{
					...
					if (gui != null)
						gui.DisplaySmallMessage("Error al spawnear especial", Color.White, true, false);
				}
			}
			return true;
		}
