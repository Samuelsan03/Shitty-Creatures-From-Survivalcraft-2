...
namespace Game
{
	public class ComponentBoomerExplosion : Component, IUpdateable
	{
		...
		public void CreateExplosionImmediately()
		{
			...
			float lureRange = 15f; // Rango base de atracción
			switch (difficulty)
			{
			...
			{
				if (finalShockwaveDamage > 0 && finalEntityRadius > 0) DamageNearbyEntities(position, finalShockwaveDamage, finalShockwaveForce, finalEntityRadius);
				if (DestroyBlocks && finalBlockRadius > 0 && m_subsystemTerrain != null) DamageNearbyBlocks(position, finalBlockRadius);
			}
		...
		public override void OnEntityRemoved()
		{
			...
			if (!PreventExplosion && !m_exploded && m_componentBody != null) CreateExplosionImmediately();
		}
	}
}