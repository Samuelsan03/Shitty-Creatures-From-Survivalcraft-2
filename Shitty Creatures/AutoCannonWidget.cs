...
public class AutoCannonWidget : CanvasWidget
{
	public AutoCannonWidget(ComponentPlayer componentPlayer, int slotIndex)
	{
		...
		// Buscar etiquetas por nombre y configurar texto localizado
		var titleLabel1 = this.Children.Find<LabelWidget>("AutoCannonTitleLabel", true);
		var titleLabel2 = this.Children.Find<LabelWidget>("InventoryTitleLabel", true);
		var speedLabel = this.Children.Find<LabelWidget>("ProjectileSpeedLabel", true);
		var rateLabel = this.Children.Find<LabelWidget>("ShotsPerSecondLabel", true);
		var spreadLabel = this.Children.Find<LabelWidget>("ProjectileSpreadLabel", true);
		var fuelLabel = this.Children.Find<LabelWidget>("FuelLabel", true);

		if (titleLabel1 != null)
			titleLabel1.Text = LanguageControl.Get("ContentWidgets", "AutoCannonWidget", "Title", "Auto-Cannon");

		if (titleLabel2 != null)
			titleLabel2.Text = LanguageControl.Get("ContentWidgets", "AutoCannonWidget", "InventoryTitle", "Inventory");

		if (speedLabel != null)
			speedLabel.Text = LanguageControl.Get("ContentWidgets", "AutoCannonWidget", "ProjectileSpeed", "Projectile Speed");

		if (rateLabel != null)
			rateLabel.Text = LanguageControl.Get("ContentWidgets", "AutoCannonWidget", "ShotsPerSecond", "Shots Per Second");

		if (spreadLabel != null)
			spreadLabel.Text = LanguageControl.Get("ContentWidgets", "AutoCannonWidget", "ProjectileSpread", "Projectile Spread");

		if (fuelLabel != null)
			fuelLabel.Text = LanguageControl.Get("ContentWidgets", "AutoCannonWidget", "Fuel", "Fuel");

		// CORREGIDO: Usar "ok" y "cancel" que son las claves en tu JSON
		if (this.m_okButton != null)
			this.m_okButton.Text = LanguageControl.Get("Usual", "ok", "OK");

		if (this.m_cancelButton != null)
			this.m_cancelButton.Text = LanguageControl.Get("Usual", "cancel", "X");
