...
namespace Game
{
	public class SubsystemSCARBehavior : SubsystemBlockBehavior
	{
		public override int[] HandledBlocks
		{
			get
			{
				return new int[]
				{
					BlocksManager.GetBlockIndex(typeof(SCARBlock), true, false)
				};
			}
		}
		...
		public override bool OnAim(Ray3 aim, ComponentMiner componentMiner, AimState state)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					...
					int num2 = slotValue;
					bool flag3 = num == BlocksManager.GetBlockIndex(typeof(SCARBlock), true, false) && slotCount > 0;
					if (flag3)
					{
						double gameTime;
						...
						// SCAR tiene muy poco retroceso en la vida real
						Vector3 v = (float)((componentMiner.ComponentCreature.ComponentBody.IsSneaking ? 0.007 : 0.022) + 0.12 * (double)MathUtils.Saturate((num3 - 6f) / 40f)) * new Vector3
						{
						...
						if (flag5)
						{
							bool flag6 = 0.1f * num3 < 0.8f;
							if (flag6)
							{
								vector.Y += 0.05f * (num3 - 1f);
							}
							else
							{
								vector.Y += 0.2f;
						...
						vector = Vector3.Normalize(vector + v * 1.2f);
						switch (state)
						{
							case AimState.InProgress:
								{
									...
									if (flag7)
									{
										...
										componentFirstPersonModel.ItemOffsetOrder = new Vector3(-0.2f, 0.14f, 0.07f);
									...
									componentMiner.ComponentCreature.ComponentCreatureModel.InHandItemOffsetOrder = new Vector3(-0.075f, -0.07f, 0.06f);
									componentMiner.ComponentCreature.ComponentCreatureModel.InHandItemRotationOrder = new Vector3(-1.6f, 0f, 0f);
									int bulletNum = SCARBlock.GetBulletNum(Terrain.ExtractData(slotValue));
									bool flag9 = bulletNum > 0;
									bool flag10 = this.m_subsystemTime.PeriodicGameTimeEvent(0.1, 0.0) && flag9; // 600 RPM = 0.1s entre disparos
									if (flag10)
									{
										Vector3 vector2 = componentMiner.ComponentCreature.ComponentCreatureModel.EyePosition + componentMiner.ComponentCreature.ComponentBody.Matrix.Right * 0.28f - componentMiner.ComponentCreature.ComponentBody.Matrix.Up * 0.18f;
										Vector3 vector3 = Vector3.Normalize(vector2 + vector * 12f - vector2);
										...
										int num5 = 1;
										Vector3 vector5 = new Vector3(0.01f, 0.01f, 0.03f); // SCAR es muy preciso
										...
										this.m_subsystemParticles.AddParticleSystem(new GunFireParticleSystem(this.m_subsystemTerrain, vector2 + 1.4f * vector6, vector6), false);
										this.m_subsystemNoise.MakeNoise(vector2, 0.85f, 45f); // Más alcance de sonido
										int bulletNum2 = bulletNum - 1;
										num2 = Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(SCARBlock), true, false), 0, SCARBlock.SetBulletNum(bulletNum2));
		...
		{
			bool flag = value == BlocksManager.GetBlockIndex(typeof(SCARBulletBlock), true, false) && SCARBlock.GetBulletNum(Terrain.ExtractData(inventory.GetSlotValue(slotIndex))) < 30;
			bool flag2 = flag;
			int result;
			if (flag2)
			{
		...
		public override void ProcessInventoryItem(IInventory inventory, int slotIndex, int value, int count, int processCount, out int processedValue, out int processedCount)
		{
			...
			int bulletNum = SCARBlock.GetBulletNum(Terrain.ExtractData(inventory.GetSlotValue(slotIndex)));
