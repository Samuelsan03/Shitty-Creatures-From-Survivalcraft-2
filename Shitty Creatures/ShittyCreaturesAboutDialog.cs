...
namespace Game
{
	public class ShittyCreaturesAboutDialog : Dialog
	{
		...
		{
			// Efecto arcoíris en el texto del botón
			m_rainbowHue = (m_rainbowHue + 1.5f) % 360f;
			Vector3 hsv = new Vector3(m_rainbowHue, 1f, 1f);
			Vector3 rgb = Color.HsvToRgb(hsv);
			m_buttonLabel.Color = new Color((int)(rgb.X * 255f), (int)(rgb.Y * 255f), (int)(rgb.Z * 255f), 255);
