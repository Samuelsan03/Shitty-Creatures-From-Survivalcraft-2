...
namespace Game
{
	public class Healing : ComponentBehavior, IUpdateable
	{
		...
		/// <summary>
		/// Verifica si una criatura está contratada (si tiene el componente hireable)
		/// </summary>
		private bool IsCreatureHired(ComponentCreature creature)
		{
			...
			if (hireable != null)
				return hireable.IsHired;

			// Si no tiene el componente hireable, es una criatura natural del rebaño
			return true;
		}
		...
		private bool NeedsSelfHealing()
		{
			...
			// Verificar si el sanador está contratado (si aplica)
			if (!IsCreatureHired(m_componentCreature))
				return false;
		...
		void SetupStateMachine()
		{
			...
			m_stateMachine.AddState("SelfHealing", () =>
			{
				...
				// Verificar si sigue contratado antes de iniciar auto-curación
				if (!IsCreatureHired(m_componentCreature))
				{
					m_stateMachine.TransitionTo("Idle");
					return;
			...
			}, () =>
			{
				...
				// Verificar si sigue contratado durante auto-curación
				if (!IsCreatureHired(m_componentCreature))
				{
					m_componentCreatureModel.AimHandAngleOrder = 0f;
					if (m_selfHealParticles != null)
				...
				if (elapsed >= 1.5)
				{
					...
					// Solo restaurar salud completa si estaba herida
			...
			m_stateMachine.AddState("HealingAllies", () =>
			{
				...
				foreach (var ally in m_healTargets)
				{
					...
					// Verificar que el aliado siga contratado
			...
			}, () =>
			{
				...
				// Remover aliados que ya no son válidos (muertos o no contratados)
				...
				if (!m_alliesHealApplied && elapsed >= 1.5)
				{
					foreach (var ally in m_healTargets)
					{
						...
						// Verificar que siga contratado al momento de curar
						if (!IsCreatureHired(ally))
							continue;
						...
						// Verificar nuevamente que siga contratado
						if (!IsCreatureHired(ally))
							continue;
		...
		void FindAllHurtOrDiseasedAllies(List<ComponentCreature> list)
		{
			...
			foreach (ComponentCreature creature in Project.FindSubsystem<SubsystemCreatureSpawn>(true).Creatures)
			{
				...
				// VERIFICACIÓN PRINCIPAL: Solo curar criaturas contratadas
		...
		bool CureCreatureDiseases(ComponentCreature creature, ComponentCreature healer)
		{
			...
			// Verificar que esté contratado antes de curar enfermedades
			if (!IsCreatureHired(creature))
				return false;
