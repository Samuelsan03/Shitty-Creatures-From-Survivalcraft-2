...
namespace Game
{
	public class SubsystemAUGBehavior : SubsystemBlockBehavior
	{
		public override int[] HandledBlocks
		{
			get
			{
				return new int[]
				{
					BlocksManager.GetBlockIndex(typeof(AUGBlock), true, false)
				};
			}
		}
		...
		public override void Load(ValuesDictionary valuesDictionary)
		{
			...
			this.fire = true;
			// Inicializar el índice de balas
		...
		public override bool OnAim(Ray3 aim, ComponentMiner componentMiner, AimState state)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					...
					int num2 = slotValue;
					bool flag3 = num == BlocksManager.GetBlockIndex(typeof(AUGBlock), true, false) && slotCount > 0;
					if (flag3)
					{
						double gameTime;
							...
							case AimState.InProgress:
								{
									...
									int bulletNum = AUGBlock.GetBulletNum(Terrain.ExtractData(slotValue));
									...
									if (flag10)
									{
										...
										{
											// Obtener el índice de la bala actual (ciclado entre las 6 balas)
											int bulletBlockIndex = this.GetCurrentBulletIndex();

											// Crear el valor del bloque de bala
											...
											// Cambiar a la siguiente bala para el próximo disparo
											this.m_bulletIndex = (this.m_bulletIndex + 1) % 6;
										}
										this.m_subsystemAudio.PlaySound("Audio/Armas/AUG fuego", 1f, this.m_random.Float(-0.0001f, 0.00001f), componentMiner.ComponentCreature.ComponentCreatureModel.EyePosition, 10f, true);
										...
										num2 = Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(AUGBlock), true, false), 0, AUGBlock.SetBulletNum(bulletNum2));
									...
									else
									{
										...
										if (flag13)
										{
											...
											{
												string bulletName = LanguageControl.Get("Blocks", "AUGBullet:0", "DisplayName");
		...
		{
			bool flag = value == BlocksManager.GetBlockIndex(typeof(AUGBullet), true, false) && AUGBlock.GetBulletNum(Terrain.ExtractData(inventory.GetSlotValue(slotIndex))) < 30;
			bool flag2 = flag;
			int result;
			if (flag2)
			{
		...
		public override void ProcessInventoryItem(IInventory inventory, int slotIndex, int value, int count, int processCount, out int processedValue, out int processedCount)
		{
			...
			int bulletNum = AUGBlock.GetBulletNum(Terrain.ExtractData(inventory.GetSlotValue(slotIndex)));
			...
			if (flag2)
			{
				...
				inventory.AddSlotItems(slotIndex, Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(AUGBlock), true, false), 0, AUGBlock.SetBulletNum(30)), 1);
		...
		// Método para obtener el índice del bloque de bala actual
		...
		// Variable para controlar qué bala se usa actualmente
