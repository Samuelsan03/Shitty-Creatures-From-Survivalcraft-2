...
namespace Game
{
	public class SubsystemFamasBehavior : SubsystemBlockBehavior
	{
		public override int[] HandledBlocks
		{
			get
			{
				return new int[]
				{
					BlocksManager.GetBlockIndex(typeof(FamasBlock), true, false)
				};
			}
		}
		...
		public override bool OnAim(Ray3 aim, ComponentMiner componentMiner, AimState state)
		{
			...
			if (flag)
			{
				...
				if (flag2)
				{
					...
					int num2 = slotValue;
					bool flag3 = num == BlocksManager.GetBlockIndex(typeof(FamasBlock), true, false) && slotCount > 0;
					if (flag3)
					{
						double gameTime;
						...
						Vector3 v = (float)((componentMiner.ComponentCreature.ComponentBody.IsSneaking ? 0.005 : 0.02) + 0.15 * (double)MathUtils.Saturate((num3 - 4f) / 30f)) * new Vector3
						{
						...
						if (flag5)
						{
							...
							if (flag6)
							{
								vector.Y += 0.012f * (num3 - 1f); // 0.03f * 0.4f
							}
							else
							{
								vector.Y += 0.036f; // 0.03f * 1.2f
							}
						}
							...
							case AimState.InProgress:
								{
									...
									int bulletNum = FamasBlock.GetBulletNum(Terrain.ExtractData(slotValue));
									bool flag9 = bulletNum > 0;
									// FAMAS tiene cadencia de 900-1100 RPM, usamos 0.09s (aproximadamente 667 RPM) para balance
									bool flag10 = this.m_subsystemTime.PeriodicGameTimeEvent(0.09, 0.0) && flag9;
									if (flag10)
									{
										...
										int num5 = 1;
										Vector3 vector5 = new Vector3(0.012f, 0.012f, 0.04f);
										...
										this.m_subsystemAudio.PlaySound("Audio/Armas/FAMAS fuego", 1.2f, this.m_random.Float(-0.05f, 0.05f), componentMiner.ComponentCreature.ComponentCreatureModel.EyePosition, 12f, true);
										...
										num2 = Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(FamasBlock), true, false), 0, FamasBlock.SetBulletNum(Terrain.ExtractData(slotValue), bulletNum2));
									...
									else
									{
										...
										if (flag13)
										{
											...
											{
												string bulletName = LanguageControl.Get("Blocks", "FamasBulletBlock:0", "DisplayName");
		...
		public override int GetProcessInventoryItemCapacity(IInventory inventory, int slotIndex, int value)
		{
			...
			int bulletNum = FamasBlock.GetBulletNum(slotData);

			if (value == BlocksManager.GetBlockIndex(typeof(FamasBulletBlock), true, false) && bulletNum < 30)
			{
				return 1;
			}
		...
		public override void ProcessInventoryItem(IInventory inventory, int slotIndex, int value, int count, int processCount, out int processedValue, out int processedCount)
		{
			...
			if (bulletNum < 30)
			{
				processedValue = 0;
				processedCount = 0;
				int newData = FamasBlock.SetBulletNum(slotData, 30);
				inventory.RemoveSlotItems(slotIndex, 1);
				inventory.AddSlotItems(slotIndex,
					Terrain.MakeBlockValue(BlocksManager.GetBlockIndex(typeof(FamasBlock), true, false), 0, newData), 1);
