...
namespace Game
{
	public class PirateTraderWidget : CanvasWidget
	{
		...
		public PirateTraderWidget(ComponentPirateTrader trader, ComponentInventory playerInventory, Entity npcEntity)
		{
			m_trader = trader;
			...
			if (m_inventoryGrid != null && m_playerInventory != null)
			{
				...
				// Mostrar TODOS los slots del inventario del jugador
				// El inventario del jugador tiene slots 0-25
				// Mostrar solo los slots del inventario principal (no la barra rápida)
			...
			if (m_traderInventory.SlotsCount > 0)
			{
				m_selectedIndex = 0;
				UpdateSelection();
			}

			if (m_trader != null) m_trader.TradeCompleted += OnTradeCompleted;
		}
			...
			if (m_buyButton != null && m_buyButton.IsClicked)
			{
				if (m_selectedIndex >= 0 && m_selectedIndex < m_trader.TradeItems.Count)
				{
					if (m_playerCoinInventory != null && m_trader.IsItemAvailable(m_selectedIndex))
					{
						...
						bool success = m_trader.TryBuyItem(m_playerInventory, m_selectedIndex, m_playerCoinInventory, out purchasedItemValue);
						if (success)
						{
