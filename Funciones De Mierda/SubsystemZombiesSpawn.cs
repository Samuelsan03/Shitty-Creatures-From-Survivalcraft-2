...
namespace Game
{
    public class SubsystemZombiesSpawn : Subsystem, IUpdateable
    {
        ...
        // Control de spawn continuo para mantener la noche activa
        private bool m_continuousSpawnMode = false;
        ...
        private void SpawnZombiesFromWave()
        {
            ...
            bool hasBosses = m_currentWaveSpawns.Keys.Any(key => m_bossZombieTypes.Contains(key));

            // Si ya no quedan spawns en la oleada, activamos modo continuo
            if (m_currentWaveSpawns.Values.Sum() == 0)
            {
                if (!m_continuousSpawnMode)
                {
                    m_continuousSpawnMode = true;
                }
            }
            ...
            if (m_continuousSpawnMode)
            {
                // En modo continuo, usamos los tipos de zombies normales que ya existen en los TXT
                // pero sin consumir del contador de la oleada
                var regularTypes = m_originalWaveSpawns
                    .Where(kv => !m_bossZombieTypes.Contains(kv.Key))
                    .Select(kv => new KeyValuePair<string, int>(kv.Key, 1))
                    .ToList();

                // Si no hay tipos regulares en la oleada, usamos una lista básica
                if (regularTypes.Count == 0)
                {
                    regularTypes = new List<KeyValuePair<string, int>>
                    {
                        new KeyValuePair<string, int>("InfectedNormal1", 1),
                        new KeyValuePair<string, int>("InfectedNormal2", 1),
                        new KeyValuePair<string, int>("InfectedFast1", 1),
                        new KeyValuePair<string, int>("InfectedFast2", 1)
                    };
                }

                availableTemplates = regularTypes;
            }
            else
            {
            ...
            for (int attempt = 0; attempt < 3; attempt++)
            {
                ...
                foreach (GameWidget gameWidget in this.m_subsystemViews.GameWidgets)
                {
                    ...
                    if (spawnPoint.HasValue)
                    {
                        ...
                        if (entity != null)
                        {
                            // Solo decrementamos si NO estamos en modo continuo
                            if (!m_continuousSpawnMode)
                            {
        ...
        public override void Load(ValuesDictionary valuesDictionary)
        {
            ...
            this.m_continuousSpawnMode = false;
