...
namespace Game
{
	public class ComponentDefensiveAI : ComponentBehavior, IUpdateable
	{
		...
		private float m_itemsLauncherReloadTime = 1.2f;
		private float m_itemsLauncherMaxShots = 0f;
		private int m_itemsLauncherShotsFired = 0;
		private bool m_isItemsLauncherReloading = false;
		private double m_itemsLauncherReloadStartTime;
		...
		private void ShootBowArrow(ComponentCreature target)
		{
			...
			{
				Vector3 firePosition = m_componentCreature.ComponentCreatureModel.EyePosition;
				firePosition += m_componentCreature.ComponentBody.Matrix.Forward * 0.5f;
				firePosition.Y -= 0.05f;
				...
				float currentAccuracy = 0.025f * (1.5f - m_currentDraw);
				direction += new Vector3(
					m_random.Float(-currentAccuracy, currentAccuracy),
					m_random.Float(-currentAccuracy * 0.3f, currentAccuracy * 0.3f),
					m_random.Float(-currentAccuracy, currentAccuracy)
				);
				direction = Vector3.Normalize(direction);

				float speedMultiplier = 0.5f + (m_currentDraw * 1.5f);
				float currentSpeed = 38f * speedMultiplier;
		...
		private void ShootCrossbowBolt(ComponentCreature target)
		{
			...
			{
				Vector3 firePosition = m_componentCreature.ComponentCreatureModel.EyePosition;
				firePosition += m_componentCreature.ComponentBody.Matrix.Forward * 0.4f;
				...
				float speed = 55f * (0.9f + (m_currentDraw * 0.3f));
		...
		private void ShootRepeatCrossbowArrow(ComponentCreature target)
		{
			...
			{
				Vector3 firePosition = m_componentCreature.ComponentCreatureModel.EyePosition;
				firePosition += m_componentCreature.ComponentBody.Matrix.Forward * 0.45f;
				firePosition.Y -= 0.02f;
