...
namespace Game
{
	public class ComponentNewVitalStats : ComponentVitalStats
	{
		...
		private bool IsDrinkableBowl(Block block)
		{
			return block is WaterBowlBlock ||
				   block is BoiledWaterBowlBlock ||
				   block is CactusJuiceBowlBlock ||
				   block is AntidoteBowlBlock ||
				   block is TeaAntifluBowlBlock;
		}
		...
		public override bool Eat(int value)
		{
			...
			else if (IsDrinkableBowl(block))
			{
				return DrinkFromBowl(value);
			}
			if (IsFruit(block))
			{
		...
		public virtual bool DrinkFromBowl(int value)
		{
			Block block = BlocksManager.Blocks[Terrain.ExtractContents(value)];
			float thirstRestore = 0f;
			bool causesSickness = false;
			bool isAntidote = false;
			bool isAntiflu = false;
			string messageKey = "DrankDrinkableWater";
			var playerInventory = m_componentPlayer?.ComponentMiner?.Inventory;
			int activeSlot = playerInventory != null ? playerInventory.ActiveSlotIndex : -1;
			bool fromDrag = false;
			if (m_componentGui != null)
			...
			if (block is WaterBowlBlock)
			{
				thirstRestore = 0.15f;
				causesSickness = true;
			}
			else if (block is BoiledWaterBowlBlock)
			{
				thirstRestore = 0.25f;
				messageKey = "DrankBoiledWater";
			}
			else if (block is CactusJuiceBowlBlock)
			{
				thirstRestore = 0.20f;
			}
			else if (block is AntidoteBowlBlock)
			{
				thirstRestore = 0f;
				messageKey = "DrankAntidote";
				isAntidote = true;
			}
			else if (block is TeaAntifluBowlBlock)
			{
				thirstRestore = 0f;
				messageKey = "DrankAntiflu";
				isAntiflu = true;
			...
			if (playerInventory != null)
			{
				int emptyBowlIndex = BlocksManager.GetBlockIndex<EmptyBowlBlock>(throwIfNotFound: true);
				int emptyBowlValue = Terrain.MakeBlockValue(emptyBowlIndex, 0, 0);
				if (fromDrag)
				{
					playerInventory.AddSlotItems(activeSlot, emptyBowlValue, 1);
				}
				else
				{
					int slotCount = playerInventory.GetSlotCount(activeSlot);
			...
			m_subsystemAudio?.PlaySound("Audio/UI/drinking", 1f, 0f, 0f, 0f);
			Color messageColor;
			if (messageKey == "DrankBoiledWater")
			{
				messageColor = new Color(80, 80, 255);
			}
			else if (messageKey == "DrankAntidote" || messageKey == "DrankAntiflu")
